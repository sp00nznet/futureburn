...
static int DvdVideoAuthorCommand(string[] args)
{
    ...
    // Probe the input and show the plan.
    Futureburn.Core.Authoring.MkvDvdPipeline.Probed probed;
    try { probed = Futureburn.Core.Authoring.MkvDvdPipeline.Probe(input); }
    catch (Exception ex)
    {
    ...
    Console.WriteLine($"  System:    {(isPal ? "PAL" : "NTSC")}  ({probed.Aspect})");
    ...
    // Author — the pipeline does transcode + spumux + dvdauthor.
    Futureburn.Core.Authoring.MkvDvdPipeline.AuthorResult result;
    try
    {
        int lastPct = -10;
        result = Futureburn.Core.Authoring.MkvDvdPipeline.Author(
            ...
            onLog: line =>
            {
                ...
                if (!line.StartsWith("frame=") && !line.StartsWith("STAT:"))
    ...
    if (!result.UsedDvdauthor)
    {
        ...
        Console.WriteLine("  Install DVDStyler (winget install AlexThuering.DVDStyler) for real IFOs.");
    }
...
static int DvdauthorInfo()
{
    ...
    if (info is null)
    {
        ...
        Console.WriteLine("  Not found.");
        Console.WriteLine();
        Console.WriteLine("  Install via one of (in approximate order of ease on Windows):");
        Console.WriteLine();
        Console.WriteLine("    winget install AlexThuering.DVDStyler");
        Console.WriteLine("       (DVDStyler bundles dvdauthor.exe — and is also a useful");
        Console.WriteLine("        DVD-Video authoring GUI in its own right)");
        Console.WriteLine();
        Console.WriteLine("    scoop bucket add extras; scoop install dvdauthor");
        Console.WriteLine();
        Console.WriteLine("    Direct download: https://dvdauthor.sourceforge.net/");
        Console.WriteLine();
        Console.WriteLine("  (Chocolatey doesn't currently package dvdauthor.)");
        Console.WriteLine();
