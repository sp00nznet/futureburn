...
- ✅ **DVD-Video authoring (experimental)** — `dvdv-author <input> <out>` runs ffmpeg's `ntsc-dvd` / `pal-dvd` preset to produce MPEG-2 + AC-3 in DVD-PS, places it as `VTS_01_1.VOB`, and writes the standard `VIDEO_TS\` + (empty) `AUDIO_TS\` folder structure with skeleton IFO/BUP files. **Plays in VLC** and other software DVD readers. Standalone hardware players probably won't accept it (skeleton IFOs lack the navigation tables). For production-quality DVDs use DVDStyler / DVDFlick / dvdauthor and `burn-folder` the result.
